   ...
   begin
       ...
       wait until rising_edge(trig_attr);
        ...
        if ((( init41h_tmp(15 downto 12) = "0001" or init41h_tmp(15 downto 12) = "0010") and init46h_tmp(3 downto 0)/="0000") or --single pass or cont mode and vuser selected
          ...
          assert false report "[Unisim SYSMONE1-177] The atrributes INIT_40 and INIT_41 are set such that  in single pass, continuous mode, or single channel mode, VUSER is selected. In Kintex devices, SYSMONE1 model has no way of knowing whether VUSER has a 6V range or a 3V range and assumes all channels have 3V range. If HR banks are selected, the value stored in the model's registers will be different than that of the actual hardware; and only for this specific case VUSER simulation checks should be ignored."
          severity warning;
        end if;
