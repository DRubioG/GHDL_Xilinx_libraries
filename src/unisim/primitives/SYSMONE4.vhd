...
--      SYSMON_VUSER3_MONITOR_NONE;
  ...
  begin
   ...
   DEFAULT_CHECK : process
      ...
      variable  v_str_time_now    : string(1 to 30);
      variable  v_str_time_length : integer;
      variable v_str : string(1 to 100);
   begin

      wait until rising_edge(trig_attr);

      v_str := time'image(now);
      v_str_time_length := v_str'length;
      v_str_time_now := (others => ' ');
      v_str_time_now (1 to v_str_time_length) := time'image(now);
      if (v_str_time_now(1 to 4)/="1 ps") then --Time is "0 ns", but doesn
        report "[Unisim "&string'(MODULE_NAME)&"-69] SYSMONE4 time resolution has been overridden. It should be left as picoseconds. The model will not function correctly this way."
        severity warning;
      end if;
      report "v_str_time_now="&v_str_time_now;
  ...
  begin
    if(rising_edge(DCLK_in) or rising_edge(sysmon_rst)) then
      if (sysmon_rst = '1') then
        ...
        elsif( chan_asrt_6 = '1' or (channel_out=std_logic_vector(to_unsigned(8, channel_out'length)) and busy_end_out_d='1' ) ) then
...
begin
  ...
  conv_track_ed    <= '1' when ((ed_counter = 0 or ed_counter = 22) and channel_out/=std_logic_vector(to_unsigned(8, channel_out'length))) else '0'; 
...
-------------------------------------------------------------------------------
  ...
  -------------------------------------------------------------------------------
  adcclk_period_end   <= '1' when (adcclk_count = to_integer(unsigned(curr_clkdiv_sel))-1)  else '0';
    ...
    if(rising_edge(DCLK_in)or rising_edge(sysmon_rst)) then
      if(sysmon_rst='1') then
        ...
        if(adc_state=ST_A_CALIB or adc_state=ST_A_FIRST_CALIB) then
          ...
          conversion_before_calib <= B2S((CHANNEL_out /= std_logic_vector(to_unsigned(8, channel_out'length))) and
         ...
         hr_final_conversion      <= B2S((((hr_tot_chan=1 and lr_calib_on='0') or (CHANNEL_out/=std_logic_vector(to_unsigned(8, channel_out'length)))) and (conv_hr_count=(hr_tot_chan-1))) or
                                    (single_chan_mode and (CHANNEL_out/=std_logic_vector(to_unsigned(8, channel_out'length)))));
         tot_final_conversion     <= B2S((CHANNEL_out/=std_logic_vector(to_unsigned(8, channel_out'length))) and (conv_tot_count=tot_per-1));
